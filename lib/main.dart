...
void main() async {
  ...
  MultiProvider(
    providers: [
      ...
      ChangeNotifierProvider(
        create: (context) => ShoppingCarProvider(),
      ),
      ChangeNotifierProvider(
...
class SalesmanContent extends StatefulWidget {
  ...
  @override
  _SalesmanContentStatefulState createState() => _SalesmanContentStatefulState();
}
...
class _SalesmanContentStatefulState extends State<SalesmanContent> {
  ...
  Future<void> _search(bool isAdd) async {
    ...
    for (var car in _caracteristicas) {
      carEntries.add(MapEntry("car${indexCar + 1}", controladores[indexCar++].text));
    }
                              ...
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  image: DecorationImage(
                                    fit: BoxFit.cover,
                                    image: NetworkImage('http://selkisbolivia.com/${product.image}'),
                                  ),
                                  border: Border.all(
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                        ...
                                        children: [
                                          WidgetSpan(
                                            child: Container(
                                              ...
                                              decoration: BoxDecoration(
                                                color: (double.parse(product.stock)>0)? Color(0xfffd7e14) : Color(0xff515c69),
                                                borderRadius: BorderRadius.circular(5),
                                              ),
                                              child: GestureDetector(
                                                onTap: (){
                                                  debugPrint("test tab....${product.idproducto} ");
                                                  ...
                                                  final snackBar = SnackBar(
                                                    content: Text('Se añadio el producto ${product.nombre} al carrito de compras'),
                                                ...
                                                child: Icon(Icons.shopping_cart, color: Color(0xffffffff), size: 30),
                                              ),
                                            )
                                          ),
                                          TextSpan(
                                    ...
                                    SizedBox(
                                      height: product.nombre.length > 50? 50 : 25,
                                      child: SingleChildScrollView(
                                        child: Column(
                                          children: [
                                            Text(product.nombre.length > 50? "${product.nombre.substring(0,50)}..." : product.nombre),
  ...
  void _showModalBottomSheet(BuildContext context) {
    ...
    for (int i = 0; i < _caracteristicas.length; i += 2) {
      paresDeDatos.add(_caracteristicas.sublist(
          i, i + 2 > _caracteristicas.length ? _caracteristicas.length : i + 2));
    }
